...
import { ChangeSearchDocStrategy } from './change-search-doc-strategy';
...
export class ActionContext {
  ...
  constructor(
    ...
    private changeSearchDocStrategy: ChangeSearchDocStrategy,
  ...
  private hasChangedStrategy(message: string, language: string): boolean {
    ...
    if (message === this.changeSearchDocStrategy.getStartSignal(language)) {
      strategy = this.changeSearchDocStrategy;
    }
  ...
  private isFinishSignal(message: string, language: string): boolean {
    if (
      ...
      message === this.changeSearchDocStrategy.getEndSignal(language)
    ) {
      if (this.currentStrategy) {
        this.speechSynthesizer.speak(
